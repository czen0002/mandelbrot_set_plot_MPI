...
int main(int argc, char** argv){
	...
	FILE * fp;
	char *filename = "Mandelbrot_points.ppm";
    ...
    int points_remained;
    int points_per_processor;
    points_remained = iXmax * iYmax % world_size;
    points_per_processor = iXmax * iYmax / world_size;
    ...
    if (points_remained != 0){
        local_colors = (unsigned char*)malloc((points_per_processor+1)*(int)sizeof(unsigned char)*3);
    } else {
        local_colors = (unsigned char*)malloc(points_per_processor*(int)sizeof(unsigned char)*3);
    }

    int x;
    int y;
    int offset = 0;
    // each processor computes mandelbrot set
    for (int i = world_rank; i < iXmax*iYmax; i+= world_size) {
        x = i % iXmax;
        y = i / iXmax;
        compute_mandelbrot(color, x, y, iXmax, iYmax);
    ...
    if (world_rank == 0) {
        if (points_remained != 0){
            global_colors = (unsigned char*)malloc((points_per_processor+1) * world_size * 3 *(int)sizeof(unsigned char));
        } else {
    ...
    if (points_remained != 0){
        for (int i = 0; i < (points_per_processor+1); i++) {
            MPI_Gather(local_colors+localOffset, 3, MPI_UNSIGNED_CHAR, global_colors+pointOffset, 3, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
            localOffset += 3;
            pointOffset += 3*world_size;
        }
    } else {
